...
async function generateSummary(prompt) {
  try {
    const response = await openai.createCompletion({
      ...
      prompt: prompt,
      max_tokens: 1024, // Adjust the number of tokens based on the desired length of the summary
      n: 1,
      stop: null,
      temperature: 0.2,
    });
...
function createPrompt(pullRequestDetails) {
  ...
  const prompt = `Please summarize the changes in the following pull request. Base the summary on the provided diff and title, and try to be clear and specific.\n\nTitle: ${title}\nDiff:\n${summarizedDiff}\n`;
